...
from core.messages import FlagError, FlagInfo
...
#         _url = self.get_url('flag:comment-flag', self.comment.id)
...
#         """Test backward compatibilty for this update"""
#         _url = self.get_url('flag:comment-flag', self.comment.id)

#         data = self.flag_data.copy()
#         # delete the flag object
#         self.comment.flag.get().delete()
...
#         _url = self.get_url('flag:comment-flag', self.comment_2.id)
#         data = {}
#         response = self.client.post(_url, data=data)
#         response_data = {
#             'data': {
...
#         url = self.get_url('flag:comment-flag', self.comment.id).replace('?', '')
#         self.client.logout()
...
#         url = self.get_url('flag:comment-flag', self.comment.id)
...
class ChangeFlagStateViewTest(BaseCommentFlagTest):
    ...
    def setUp(self):
        super().setUp()
        self.data = {
            'state': self.comment_for_change_state.flag.get().REJECTED
        }
    ...
    #     response = self.client.post(self.get_url('flag:flag-change-state', comment.id), data=self.data)
    ...
    #         self.get_url('flag:flag-change-state', self.comment_for_change_state.id), data=self.data)
    ...
    def test_change_flag_state_with_wrong_state_value(self):
        ...
        self.client.force_login(self.moderator)
        self.assertEqual(int(self.client.session['_auth_user_id']), self.moderator.id)
        self.assertEqual(self.comment_for_change_state.flag.get().state, self.comment_for_change_state.flag.get().FLAGGED)

        # valid state is REJECTED and RESOLVED
        self.data['state'] = self.comment_for_change_state.flag.get().UNFLAGGED
        response = self.client.post(
            self.get_url('flag:flag-change-state', self.comment_for_change_state.id), data=self.data
        )
        ...
        self.assertEqual(self.comment_for_change_state.flag.state, self.comment_for_change_state.flag.get().FLAGGED)
    ...
    def test_change_flag_state_success(self):
        ...
        # valid state is REJECTED and RESOLVED
        self.data['state'] = self.comment_for_change_state.flag.get().REJECTED  
        response = self.client.post(
            self.get_url('flag:flag-change-state', self.comment_for_change_state.id), data=self.data
        )
